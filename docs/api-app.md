...
Создаёт **новую** сессию (`jti` в JWT). Ранее выданные активные сессии того же пользователя **не** отзываются: живут до `expiresAt` или до `POST /auth/logout` с этим токеном. Так повторный вход на другом устройстве / после переустановки МП не гасит ещё рабочий токен на телефоне.
