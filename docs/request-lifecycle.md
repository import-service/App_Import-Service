...
|   | Авто: марка, модель, VIN. Люк/панорама и полный привод — да/нет. **Даты предыдущего ввоза** (`previousImportDates`) и **авто в собственности** (`ownedVehicles`: название + год) — списки, без галочек. |
|   | Владелец авто = **физлицо на кого заявка**: ФИО, телефон, СНИЛС — **вручную** на каждую заявку (не из профиля). Выбор типа заявителя на форме создания **нет** (только в регистрации). |
