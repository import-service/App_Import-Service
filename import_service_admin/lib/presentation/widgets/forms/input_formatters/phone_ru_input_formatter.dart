...
/// Формат: +7 (999) 000-00-00 (как в МП).
class PhoneRuInputFormatter extends TextInputFormatter {
  ...
  /// Пусто / только «+7» → null; иначе `+7XXXXXXXXXX` или throw через validator.
  static String? normalizeOptionalForApi(String? raw) {
    var digits = digitsOnly(raw);
    if (digits.startsWith('8')) {
      digits = '7${digits.substring(1)}';
    }
  ...
  static bool isCompleteRuPhone(String? raw) {
    var digits = digitsOnly(raw);
    if (digits.startsWith('8')) {
      digits = '7${digits.substring(1)}';
    }
  ...
  static bool isEmptyRuPhone(String? raw) {
    var digits = digitsOnly(raw);
    if (digits.startsWith('8')) {
      digits = '7${digits.substring(1)}';
    }
    return digits.isEmpty || digits == '7';
  }
}
